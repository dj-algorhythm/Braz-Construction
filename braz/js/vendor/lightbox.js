(function() {
    ...
    var Lightbox = function() {
        ...
        Lightbox.prototype.enable = function() {
            var self = this;
            $("body").on("click", "div[data-lightbox]", function(event) {
        ...
        Lightbox.prototype.build = function() {
            ...
            this.$lightbox.find(".lb-next").on("click", function() {
                if (self.currentImageIndex ===
                    self.album.length - 1) self.changeImage(0);
                else self.changeImage(self.currentImageIndex + 1);
                return false
            });
        ...
        Lightbox.prototype.start = function($link) {
            ...
            function addToAlbum($link) {
                self.album.push({
                    link: $link.attr("data-original"),
                    title: $link.attr("data-title") || $link.attr("title")
                })
            }
            var dataLightboxValue = $link.attr("data-lightbox");
            var $links;
            $links = $('img[data-lightbox="' + dataLightboxValue + '"]');
        ...
        Lightbox.prototype.changeImage =
        ...
        Lightbox.prototype.updateDetails = function() {
            var self = this;
            if (typeof this.album[this.currentImageIndex].title !== "undefined" && this.album[this.currentImageIndex].title !== "") this.$lightbox.find(".lb-caption").html(this.album[this.currentImageIndex].title).fadeIn("fast").find("a").on("click", function(event) {
                location.href = $(this).attr("href")
            });
            if (this.album.length > 1 && this.options.showImageNumberLabel) this.$lightbox.find(".lb-number").text(this.options.albumLabel(this.currentImageIndex +
        ...
        Lightbox.prototype.keyboardAction = function(event) {
            ...
            if (keycode === KEYCODE_ESC || key.match(/x|o|c/)) this.end();
                ...
                "p" || keycode === KEYCODE_LEFTARROW)
                if (this.currentImageIndex !== 0) this.changeImage(this.currentImageIndex - 1);
                else {
                    if (this.options.wrapAround && this.album.length > 1) this.changeImage(this.album.length - 1)
